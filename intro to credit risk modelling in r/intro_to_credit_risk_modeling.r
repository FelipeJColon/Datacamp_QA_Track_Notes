...
# Including a loss matrix
# Change the code below such that a decision tree is constructed using a loss matrix penalizing 10 times more heavily for misclassified defaults.
tree_loss_matrix <- rpart(loan_status ~ ., method = "class", control = rpart.control(cp = 0.001),data = training_set, parms = list(loss = matrix(c(0, 10, 1, 0), ncol=2)))
...
log_5_remove_inc <- glm(loan_status ~ emp_cat + grade, family = binomial, data = training_set)
